...
void display_prompt()
{
    ...
    if (avfs_file_exists("username.cfg")) {     
        ...
        printr("default [no 'username.cfg' found]\n");
    }
...
int keyboard_input(char *userinput)
{
    static char command_buffer[COMMAND_BUFFER_SIZE];
    size_t command_length = 0;
    size_t cursor_position = 0;  // Track cursor position within the buffer
    ...
    while (true)
    {
        if (is_key_pressed())
        {
            ...
            // Ctrl + Shift + Delete
            if (scan_code == 0x2E && ctrl_pressed && shift_active)
            {
                terminal_putchar('^');
                print("Paste");
                terminal_putchar('\n');
            ...
            if (scan_code == 0x0E)
            {
                if (command_length > 0 && cursor_position > 0)
                {
...
{
    if (keyboard_send_command(0xED) != 0)
    {
        // Failed to send Set LEDs command
        return;
    }

    io_wait();

    if (keyboard_send_command(leds) != 0)
    {
        // Failed to send LED status byte
        return;
    }
...
{
    led_status ^= 0x04; // Caps Lock LED bit
...
char keyboard_to_char(uint8_t scancode, bool shift, bool caps_lock) {
    if (scancode >= sizeof(keyboard_map)) {
        return 0; // Return null character for invalid scancode
    }
    ...
    // For shifted letters, keep them capitalized.
    if (c >= 'A' && c <= 'Z' && caps_lock) {
